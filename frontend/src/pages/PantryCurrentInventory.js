...
const PantryCurrentInventory = () => {
  ...
  const fetchData = async () => {
    try {
      const apiUrl = 'http://20.118.4.46:3000/api/items/query';
    ...
    try {
      ...
      await Promise.all(
        updatedRows.map(async (dataItem) => {
          try {
            const response = await axios.put('http://20.118.4.46:3000/api/items/update', {
              ...
              editedData.map((dataItem, index) => (
                <tr key={index}>
                  ...
                  <td onClick={() => handleEdit(index)}>
                    ...
                    <input type="hidden" value={dataItem.item_id} name={`item_id_${index}`} />
